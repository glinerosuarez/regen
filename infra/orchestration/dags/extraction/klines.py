...
@task
def extract_klines(dag_run=None):
    """
    #### Extract klines task
    ...
    batch_size = 960
    klines = api_client.get_klines_data(pair, Interval.M_1, start, end, batch_size)
    klines_count = len(klines)
    db_manager.insert(klines)
    print(f"{klines_count} klines have been inserted into the database.")
